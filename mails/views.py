...
def download_calendar_view(request, secret):
    username = base64.urlsafe_b64decode(secret)
...
def download_vcard_view(request):
    ...
    mail_addresses = [
        ("{1}".format(*entry), mail_template.format(delay=entry[0], **values))
        for entry in settings.MAILBOXES
    ]
...
def user_activate_view(request, key):
    try:
        ...
        user.save()
        messages.success(
            request, "The user %s was successfully activated." % user.email
        )
...
def user_connect_view(request, key, account_id):
    try:
        ...
        messages.success(
            request,
            "The user %s was successfully connected with this account." % user.email,
        )
        tools.delete_log_entries(user.email)
    except:
...
def user_add_view(request):
    if request.POST:
        ...
        try:
            ...
            if email != "":
                tools.create_additional_user(email=email, user=request.user)

    users = tools.get_all_users_of_account(request.user)

    response = render(request, "mails/settings/user_table.html", {"users": users})
...
def user_delete_confirm_view(request, id):
    ...
    if user in users:
        return render(
            request, "mails/settings/user_delete_confirm.html", {"user": user}
        )
    else:
        return Http404
...
def settings_view(request):
    ...
    response = render(
        ...
        "mails/settings/settings.html",
        {
            ...
            "secret": base64.urlsafe_b64encode(request.user.username.encode("utf-8")),
        },
    )
...
def statistic_view(request):
    ...
    users = (
        Statistic.objects.filter(type="USER")
        .values("email")
        .annotate(Count("id"))
        .order_by("-id__count")
    )
    oblivious = (
        Statistic.objects.filter(type="OBL")
        .values("email")
        .annotate(Count("id"))
        .order_by("-id__count")
        .exclude(email__contains="rmd.io")
    )
    addresses = (
        Statistic.objects.filter(type="REC")
        .values("email")
        .annotate(Count("id"))
        .order_by("-id__count")
    )
